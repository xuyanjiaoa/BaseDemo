...
public class CameraView extends FrameLayout {
    ...
    private Bitmap crop(File outputFile, byte[] data, int rotation, int quality) {
        try {
            ...
            if (mFile != null) {
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Bitmap bitmap = crop(mFile, data, orientation, mQuality);
                        uiHandler.post(new Runnable() {
                            @Override
                            public void run() {
                                mTakingPicture.set(false);
                                callback.onPictureSave(mFile, bitmap);
                            }
                        });
