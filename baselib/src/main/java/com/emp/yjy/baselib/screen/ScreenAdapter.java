...
public class ScreenAdapter {
    ...
    @IntDef({MATCH_BASE_WIDTH, MATCH_BASE_HEIGHT})
    ...
    @Documented
    @Target(ElementType.PARAMETER)
    @IntDef({MATCH_UNIT_DP, MATCH_UNIT_PT})
     ...
     * @param matchType   根据宽度/高度适配，详情请参阅{@link MatchType}
     * @param matchUnit   尺寸单位，目前支持dp、pt两种尺寸，详情请参阅{@link MatchUnit}
     */
    @RequiresApi(Build.VERSION_CODES.ICE_CREAM_SANDWICH)
    public static void autoSize(@NonNull final Application application, final float designSize, @MatchType final int matchType, @MatchUnit final int matchUnit) {
        if (designSize <= 0) {
            throw new UnsupportedOperationException("designSize unEnable smaller than 0!");
        }
        ...
        if (mActivityLifecycleCallback == null) {
            mActivityLifecycleCallback = new Application.ActivityLifecycleCallbacks() {
                ...
                public void onActivityCreated(Activity activity, Bundle savedInstanceState) {
                    if (activity != null) {
                        if (activity instanceof ICustomScreenAdapter) {
                            ICustomScreenAdapter iCustomAdapter = (ICustomScreenAdapter) activity;
                            int matchType = iCustomAdapter.isBaseOnWidth() ? MATCH_BASE_WIDTH : MATCH_BASE_HEIGHT;
                            match(activity, iCustomAdapter.getDpSize(), matchType, MATCH_UNIT_DP);
                        } else {
                            match(activity, designSize, matchType, matchUnit);
                        }
